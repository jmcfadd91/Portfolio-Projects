...
@st.cache_data
def load_data():
    query = "SELECT trans_id, product_id, TO_CHAR(transaction_date, 'MM/DD/YYYY') as date, sale_quantity, total_price, transaction_type FROM fact_sales;"
    ...
    df_sales["date"] = df_sales["transaction_date"].dt.strftime("%Y-%m-%d") # Extract date
    df_sales["time"] = df_sales["transaction_date"].dt.strftime("%H:%M:%S") 
...
fig = px.line(filtered_df, x="date", y="total_price", title="Total Sales Over Time")
